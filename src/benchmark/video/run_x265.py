"""
Benchmark energetico x265 (HEVC) — VERSIONE PRESET MULTIPLI.

Modifiche rispetto alla v1:
  - PRESETS lista di preset x265 (escluso "medium" già fatto)
  - op_id include il preset → bitstream filename: x265_<seq>_<profile>_<preset>_crf<CRF>.265
...
def encode_x265_cmd(width, height, fps, n_frames, profile, preset, crf, out_path):
...
def run_encode(yuv_bytes, width, height, fps, n_frames, profile, preset, crf, out_path):
    cmd = encode_x265_cmd(width, height, fps, n_frames, profile, preset, crf, out_path)
...
def main():
    print("=" * 70)
    print(f"x265 VIDEO ENERGY BENCHMARK — EXTRA PRESETS")
    ...
    print(f"Next: python -u quality_x265.py > x265_quality_extra.log 2>&1 &")
