"""
Calcolo qualità per i bitstream VVC prodotti da run_vvenc.py.

Output:
  ~/tesi/results/video/vvenc_quality.csv
"""
...
from run_vvenc import VVENC_CRF_POINTS

CODEC = "vvenc"
QUALITY_CSV = RESULTS_DIR / "vvenc_quality.csv"
...
def main():
    print("=" * 70)
    print(f"VVC QUALITY COMPUTATION")
    ...
    t_start = time.time()
    n_done = 0
    n_total = len(UVG_SEQUENCES) * len(PROFILES) * len(VVENC_CRF_POINTS)
