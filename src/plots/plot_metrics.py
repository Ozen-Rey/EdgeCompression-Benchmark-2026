...
Include generazione automatica per tutte le 12 metriche visive.
"""
...
# Dizionario delle metriche aggiuntive
# Chiave: colonna CSV | Valore: (Etichetta Asse Y, Inverti Asse Y? True se un valore più basso è migliore)
EXTRA_METRICS = {
    "ssim": ("SSIM (↑ meglio)", False),
    "ms_ssim": ("MS-SSIM (↑ meglio)", False),
    "lpips": ("LPIPS (↓ meglio)", True),
    "dists": ("DISTS (↓ meglio)", True),
    "fsim": ("FSIM (↑ meglio)", False),
    "gmsd": ("GMSD (↓ meglio)", True),
    "vif": ("VIF (↑ meglio)", False),
    "haarpsi": ("HaarPSI (↑ meglio)", False),
    "dss": ("DSS (↑ meglio)", False),
    "mdsi": ("MDSI (↓ meglio)", True),
}
...
def load_quality():
    ...
    # FIX: Aggiunte tutte le potenziali 12 metriche alla lista delle colonne da estrarre
    cols = [
        "codec",
        "param",
        "image",
...
# ... (MANTIENI LE FUNZIONI fig1, fig2, fig3, fig4, fig5, fig6, fig7, fig8, fig9, fig10 IDENTICHE A PRIMA) ...
...
def fig7(aud):
    ...
    datasets, ds_labels, ds_colors = (
        ["librispeech", "esc50", "musdb"],
        ["LibriSpeech", "ESC-50", "MUSDB18"],
...
def fig8(aud):
    ...
    for codec, param in codec_order:
        ...
        j_per_s.append(s["j_per_s"].values[0])
...
def fig9():
    ...
    classical_pct, neural_pct = [72.5 / 72.5 * 100, 3.57 / 3.67 * 100], [
        56.1 / 72.5 * 100,
        3.67 / 3.67 * 100,
    ]
    x, w = np.arange(2), 0.3
...
# ====================================================================
# FIG EXTRA: Tutte le altre metriche R-D
# ====================================================================
def fig_extra_metric(dfq, metric_col, y_label, is_lower_better, fig_num):
    if metric_col not in dfq.columns:
        return
    ...
    for codec in CODEC_ORDER:
        s = avg_metric(dfq, codec, metric_col)
        if len(s) == 0:
            continue
        plotted = True
        ax.plot(
    ...
    ax.set_title(f"Curva Rate-Distortion: {metric_col.upper()}")
    ax.legend(loc="best", ncol=2)
    ax.set_xlim(0, 2.0)

    if is_lower_better:
        ax.invert_yaxis()  # Inverte l'asse Y per le metriche "minore è meglio" (es. LPIPS)

    filename = f"fig{fig_num}_rd_{metric_col}.pdf"
...
def main():
    ...
    print("\nGenerazione grafici metriche extra:")
    fig_num = 11
    for col, (y_label, lower_better) in EXTRA_METRICS.items():
        fig_extra_metric(dfq, col, y_label, lower_better, fig_num)
        fig_num += 1
